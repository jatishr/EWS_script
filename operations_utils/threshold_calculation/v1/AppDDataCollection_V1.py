...
# Version - 1.1
# Date - 5/27/2021
# Update dates and last update in the appd_metric_bt_config for the
# btconfig_program	btconfig_appdapplication	btconfig_env	btconfig_datacol_name	btconfig_overall_servicename
...
def generateauthtoken(tconfig):
    ...
    #8/4/2021 - Sushant if connection is aborted then give  more sleep time and try again
    try:
...
def getpullappdrawdata(appconfig, dataconfig, tconfig, dbdata ):
    ...
    for reptype in appconfig[datacolconfig]:
        ...
        # 5/27/2021 - Update the dates between dates, last update for combination program, application, env, datacol, reptype
        updatesqlstmt = "Update dbo.appd_metric_bt_config set btconfig_startdate = '" + str(startdatetm) + "', \
                        btconfig_enddate = '" + str(enddatetm) + "', btconfig_lastupdate ='" + str(datetime.datetime.today()) + "' \
                        where btconfig_program = '" + dbdata[0] + "' and btconfig_appdapplication ='" + dbdata[1] + "' \
                        and btconfig_env = '" + dbdata[2] + "' and btconfig_datacol_name = '" + dbdata[3] + "' and btconfig_overall_servicename ='" + dbdata[4] +"'"
        try:
            cursor.execute(updatesqlstmt)
            cnxn.commit()
        except Exception as e:
            print ("Error Updating data in appd_metric_bt_config",e)
            logappend("Error Updating data in appd_metric_bt_config " + e)
            continue
